...
def montar_dic_problema(m, n, Maquinas, Jobs, tempo, ordem, fl_inteiro, lb_cmax=0.0):
    return  {"m":m
            ,"n":n
            ,"Maquinas":Maquinas
            ,"Jobs": Jobs
...
def jsp_manne_var_cmax(modelo, Problema):
    tempo = Problema["tempo"]
    lb_cmax = Problema["lb_cmax"] #lower bound passado inicialmente para o modelo
    max_p_j=np.max(calcular_p_j(tempo))
    # Variável para representar o makespan: max entre o max_p_j e o lb inicialmente
    cmax = modelo.continuous_var(lb=max(lb_cmax, max_p_j), name="cmax")
...
def jsp_disjuntivo_manne(tempo, ordem, tempo_max = 3600, fl_inteiro=True, lb_cmax=0.0):
    ...
    Problema = montar_dic_problema(m, n, Maquinas, Jobs, tempo, ordem, fl_inteiro, lb_cmax)
    ###########################################################################
...
def jsp_disjuntivo_minla(tempo, ordem, tempo_max=3600, fl_inteiro=True, restricoes=[jsp_manne_rest_ordem_maq_job, jsp_manne_rest_precedencia, jsp_manne_rest_makespan], lb_cmax=0.0):
    if not jsp_checar_tempo_ordem(tempo, ordem):
        print("Matrizes de TEMPO e ORDEM incorretas!")
...
def jsp_disjuntivo_minla_favorito(tempo, ordem, tempo_max=3600, fl_inteiro=True, lb_cmax=0.0):
    if not jsp_checar_tempo_ordem(tempo, ordem):
        print("Matrizes de TEMPO e ORDEM incorretas!")
...
# Modelo de Liao
def jsp_liao(tempo, ordem, tempo_max = 3600, fl_inteiro=True, lb_cmax=0.0):
    if not jsp_checar_tempo_ordem(tempo, ordem):
        print("Matrizes de TEMPO e ORDEM incorretas!")
...
# Modelo de Liao
def jsp_liao_desigualdades(tempo, ordem, tempo_max = 3600, fl_inteiro=True, lb_cmax=0.0):
    if not jsp_checar_tempo_ordem(tempo, ordem):
        print("Matrizes de TEMPO e ORDEM incorretas!")
