...
def execucao_modelos(instancias, modelos, prefixo_arq = "t_minlafav_",  tempo_max=120, fl_primeira_sol = False, fl_heuristica_desabilitada=False, inst_mod_pular=[]):
    ...
    arquivo_solucao = "solucoes/"+prefixo_arq+ "_results.csv"
    f = open(arquivo_solucao, "w")
    f.write("problema;  modelo;  lb_cmax; num_jobs;  num_maquina;  fl_inteiro;  best_bound;  funcao_objetivo;  mip_relative_gap; number_of_contraints; number_of_variables; number_of_var_values; nb_iterations; nb_nodes_processed; nb_linear_nonzeros; status; time; has_hit_limit \n")
    f.close()
    df_solucoes_intermed = pd.DataFrame()
    for i in instancias: 
        id_inst = i["id"]
        ...
        lb_cmax = 0.0
        if (id_inst == 0) & (m == 3) & (n==4):
        	lb_cmax = 29.0
        elif (id_inst == 0) & (m==3) & (n==5):
        	lb_cmax = 33.0
        elif (id_inst == 0) & (m==15) & (n==15):
        	lb_cmax = 1210.0
        elif (id_inst == 1) & (m==15) & (n==15):
        	lb_cmax = 1177.0
        elif (id_inst == 2) & (m==15) & (n==15):
        	lb_cmax = 1197.0
        elif (id_inst == 3) & (m==15) & (n==15):
        	lb_cmax = 1172.0
        elif (id_inst == 5) & (m==15) & (n==15):
        	lb_cmax = 1162.0
        elif (id_inst == 6) & (m==15) & (n==15):
        	lb_cmax = 1243.0
        elif (id_inst == 7) & (m==15) & (n==15):
        	lb_cmax = 1181.0
        elif (id_inst == 9) & (m==15) & (n==15):
        	lb_cmax = 1206.0
...
#        for fl_inteiro in [False]:
            ...
            for mod in modelos:
                for lb in [0, lb_cmax]:
                    ...
                    modelo = mod(tempo, ordem, tempo_max=tempo_max, fl_inteiro=fl_inteiro, lb_cmax=lb)
                    # Desabilitar heuristica
                    if fl_heuristica_desabilitada:
                        modelo.parameters.mip.strategy.heuristicfreq = -1
                    if fl_primeira_sol:
                    ...
                    prefixo = prefixo_arq\
                            ...
                            +str(fl_inteiro)\
                            +"_"+modelo.name   
                    print("Modelo "+ prefixo + " criado! com LB do cmax = ", lb)
                    #modelos.append(modelo)
                    print("############", nome_problema+"_ZInt"+str(fl_inteiro)+"_"+modelo.name)
                    # Pulando instancias-modelo necessarios
                    if nome_problema+"_ZInt"+str(fl_inteiro)+"_"+modelo.name not in inst_mod_pular:                
